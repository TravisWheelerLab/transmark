...
static void create_ssi_index(ESLX_MSAFILE *afp);
static int synthesize_negatives_and_embed_positives(ESL_GETOPTS *go, struct cfg_s *cfg, ESL_SQ **posseqs, int npos, 
                            ESLX_MSAFILE *decoymsafp, 
...
main(int argc, char **argv)
{
  ...
  ESLX_MSAFILE *decoymsafp   = NULL; /* open decoy MSA file             */
  ...
  if (decoyfile != NULL) {
    ...
    if((status = eslx_msafile_Open(&(decoy_alphabet), decoyfile, NULL, alifmt, NULL,
                              &decoymsafp)) != eslOK) { 
    ...
    while ((status = eslx_msafile_Read(decoymsafp, &decoymsa)) != eslEOF)
    {
      if (status != eslOK) 
        eslx_msafile_ReadFailure(decoymsafp, status);
  ...
  while (num_decoy_seq_inserted < num_decoy_ORFs)
    {
      ...
      if ((status = eslx_msafile_Read(decoymsafp, &predecoymsa)) != eslOK)
        eslx_msafile_ReadFailure(decoymsafp, status);
...
{      
  ...
  char       *test_tmp_msg         = "failed to crate temp fle to hold test seq";
  ...
  for (i = 0; i < msa->nseq; i++){
      ...
      if(assignment[i] != ctrain) {
          ...
          for(j = 0; j < trainmsa->nseq; j++) {
              ...
              status = system("percent-identity-within-threshold.sh train_seq_tmpfile test_seq_tmpfile 60");
              ...
              if(status == 1)
                  printf("Training sequence %s will not be used\n", p_train_seq->name);
                  i_am_possibly_test[i] = 0;
                  esl_sqfile_Close(train_seq_fp);
 ...
 */
static void
create_ssi_index(ESLX_MSAFILE *afp)
{
  ESL_NEWSSI *ns      = NULL;
  ESL_MSA    *msa     = NULL;
  int         nali    = 0;
